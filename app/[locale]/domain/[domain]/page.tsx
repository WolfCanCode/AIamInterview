...
export default function DomainDetailPage({
  ...
  return (
    <>
      ...
      {hasChildren && (
        <div className="mb-4">
          <div className="text-main text-sm font-medium mt-4 text-center animate-fade-in">
            {t('select_subtopic')}
          </div>
          <div className="domain-children-grid animate-fade-in">
            {domainInstance?.children?.map((child: string) => (
              <button
                ...
                className={`card relative w-full rounded-xl font-black px-4 py-3 text-sm transition-all duration-200 overflow-hidden border border-white backdrop-blur text-main hover:scale-105 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-cyan-400/80 ${
                  selectedChild === child
                ...
                tabIndex={0}
              >
                <span className="relative text-main text-sm font-inherit font-bold drop-shadow-[0_0_6px_rgba(34,211,238,0.7)]">
                  {t(child)}
                </span>
                {selectedChild === child && (
